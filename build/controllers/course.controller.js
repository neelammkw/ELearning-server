...
exports.getFullCourse = exports.getCourseReviews = exports.replyToCourseReview = exports.addCourseReview = exports.replyToQuestion = exports.addVideoQuestion = exports.replyToReview = exports.addVideoReview = exports.dislikeVideo = exports.likeVideo = exports.checkLectureCompletion = exports.checkCourseEnrollment = exports.deleteCourse = exports.getAdminAllCourse = exports.getAllCourse = exports.addReplyToReview = exports.addReview = exports.addAnswer = exports.addQuestion = exports.getUserCourses = exports.getCourseByUser = exports.getAllCourses = exports.getSingleCourse = exports.editCourse = exports.uploadCourse = void 0;
        ...
        form.parse(req, async (err, fields, files) => {
            try {
                if (err) {
                    ...
                    return next(new ErrorHandler_1.default("Failed to parse form data", 400));
                }
                ...
                if (Array.isArray(fields.courseData)) {
                    ...
                    return next(new ErrorHandler_1.default("Course data is required", 400));
                }
                // Parse course data
                let courseData;
                try {
                    ...
                    return next(new ErrorHandler_1.default("Invalid course data format", 400));
                }
                // Validate required fields
                const requiredFields = ['name', 'description', 'price', 'level'];
                const missingFields = requiredFields.filter(field => !courseData[field]);
                if (missingFields.length > 0) {
                    return next(new ErrorHandler_1.default(`Missing required fields: ${missingFields.join(', ')}`, 400));
                }
                // Process thumbnail
                if (!files.thumbnail) {
                    return next(new ErrorHandler_1.default("Thumbnail is required", 400));
                }
                const thumbnail = Array.isArray(files.thumbnail) ? files.thumbnail[0] : files.thumbnail;
                const thumbnailResult = await cloudinary_1.default.v2.uploader.upload(thumbnail.filepath, {
                ...
                fs_1.default.unlinkSync(thumbnail.filepath);
                // Process demo video (if provided)
                if (files.demoUrl) {
                    const demoVideo = Array.isArray(files.demoUrl) ? files.demoUrl[0] : files.demoUrl;
                    const demoResult = await cloudinary_1.default.v2.uploader.upload(demoVideo.filepath, {
                        folder: "course-demos",
                        resource_type: "video",
                    });
                    courseData.demoUrl = {
                    ...
                    fs_1.default.unlinkSync(demoVideo.filepath);
                }
                ...
                if (courseData.courseData && Array.isArray(courseData.courseData)) {
                    for (let i = 0; i < courseData.courseData.length; i++) {
                        ...
                        if (files[fileKey]) {
                            ...
                            const videoResult = await cloudinary_1.default.v2.uploader.upload(videoFile.filepath, {
                            ...
                            fs_1.default.unlinkSync(videoFile.filepath);
                        }
                    }
                }
                // Create course
                ...
                if (files) {
                    Object.values(files).forEach(file => {
                        ...
                        filesToDelete.forEach(f => {
                            if (f.filepath && fs_1.default.existsSync(f.filepath)) {
    ...
    try {
        ...
        form.parse(req, async (err, fields, files) => {
            try {
                ...
                const existingCourse = await course_model_1.default.findById(courseId).session(session);
                if (!existingCourse) {
                ...
                if (files.demoUrl) {
                    ...
                    if (existingCourse.demoUrl?.public_id) {
                        await cloudinary_1.default.v2.uploader.destroy(existingCourse.demoUrl.public_id, { resource_type: 'video' });
                    }
                    const demoVideo = Array.isArray(files.demoUrl) ? files.demoUrl[0] : files.demoUrl;
                    const demoResult = await cloudinary_1.default.v2.uploader.upload(demoVideo.filepath, {
                        folder: "course-demos",
                ...
                if (courseData.courseData && Array.isArray(courseData.courseData)) {
                    for (let i = 0; i < courseData.courseData.length; i++) {
                        ...
                        const existingContent = existingCourse.courseData.find(c => c._id && content._id && c._id.toString() === content._id.toString());
                ...
                const updatedCourse = await course_model_1.default.findByIdAndUpdate(courseId, { $set: courseData }, {
                    new: true,
                    session,
                    populate: {
                        path: 'reviews.user',
...
exports.addAnswer = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (req.user?._id === question.user._id) {
            ...
            const html = await ejs_1.default.renderFile(path_1.default.join(__dirname, "../mails/estion-reply.ejs"), data);
            try {
...
exports.addReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!courseExists) {
            return next(new ErrorHandler_1.default("You must purchase the course to review", 403));
        }
        const course = await course_model_1.default.findById(courseId);
        if (!course) {
            return next(new ErrorHandler_1.default("Course not found", 404));
        ...
        if (!review || !rating) {
            return next(new ErrorHandler_1.default("Review and rating are required", 400));
        }
        if (rating < 1 || rating > 5) {
            return next(new ErrorHandler_1.default("Rating must be between 1 and 5", 400));
        }
        const reviewData = {
            user: req.user._id,
...
const checkCourseEnrollment = async (req, res, next) => {
    try {
        ...
        if (!userId) {
            ...
            return next(new ErrorHandler_1.default("User ID missing", 400));
        }
        if (!courseId) {
            console.error('No course ID found in request');
            return next(new ErrorHandler_1.default("Course ID missing", 400));
        }
        ...
        const isEnrolled = user.courses.some((course) => {
            if (!course.courseId) {
...
exports.checkCourseEnrollment = checkCourseEnrollment;
exports.checkLectureCompletion = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!course) {
            return next(new ErrorHandler_1.default('Course not found for this user', 404));
        }
...
// });
// Like a video
exports.likeVideo = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const contentItem = course.courseData.find(item => item._id.toString() === contentId);
        if (!contentItem) {
...
exports.dislikeVideo = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const { courseId, contentId } = req.params;
        const userId = req.user?.id;
        // Find the course
...
// Add video review
exports.addVideoReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    const session = await mongoose_1.default.startSession();
    session.startTransaction();
    try {
        const { courseId, contentId } = req.params;
        ...
        // Validate input
        if (!rating || !comment) {
            return next(new ErrorHandler_1.default("Rating and comment are required", 400));
        }
        if (rating < 1 || rating > 5) {
            return next(new ErrorHandler_1.default("Rating must be between 1 and 5", 400));
        }
        if (comment.trim().length < 10) {
            return next(new ErrorHandler_1.default("Comment must be at least 10 characters", 400));
        }
        ...
        // Find the content item
        const contentIndex = course.courseData.findIndex(item => item._id.toString() === contentId);
        if (contentIndex === -1) {
            return next(new ErrorHandler_1.default("Content not found", 404));
        }
        ...
        const existingReviewIndex = course.courseData[contentIndex].reviews.findIndex(r => r.user && r.user._id.toString() === userId.toString());
        const reviewData = {
            user: req.user,
            rating: Number(rating),
        ...
        if (existingReviewIndex >= 0) {
            course.courseData[contentIndex].reviews[existingReviewIndex] = reviewData;
        }
        else {
            course.courseData[contentIndex].reviews.push(reviewData);
        }
        ...
        const allRatings = course.courseData.flatMap(item => item.reviews?.map(review => review.rating) || []);
...
exports.replyToReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const userId = req.user?._id;
        if (!reply) {
            return next(new ErrorHandler_1.default("Reply message is required", 400));
        }
        const course = await course_model_1.default.findById(courseId);
        if (!course) {
            return next(new ErrorHandler_1.default("Course not found", 404));
        ...
        // Find the review
        const reviewIndex = course.courseData[contentIndex].reviews.findIndex((r) => r._id.toString() === reviewId);
        if (reviewIndex === -1) {
            return next(new ErrorHandler_1.default("Review not found", 404));
        }
        ...
        course.courseData[contentIndex].reviews[reviewIndex].commentReplies.push(replyData);
        // Mark as modified and save
        course.markModified("courseData");
        await course.save();
        res.status(200).json({
...
exports.addVideoQuestion = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const { courseId, contentId } = req.params;
        ...
        course.courseData[contentIndex].questions.push(questionData);
        // Mark as modified and save
        course.markModified("courseData");
        await course.save();
...
exports.replyToQuestion = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const questionIndex = course.courseData[contentIndex].questions.findIndex((q) => q._id.toString() === questionId);
        if (questionIndex === -1) {
        ...
        course.courseData[contentIndex].questions[questionIndex].questionReplies.push(replyData);
        course.markModified("courseData");
        await course.save();
...
exports.addCourseReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    const session = await mongoose_1.default.startSession();
    session.startTransaction();
    try {
...
exports.replyToCourseReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        course.reviews[reviewIndex].commentReplies.push(replyData);
...
// Get course reviews
exports.getCourseReviews = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const { courseId } = req.params;
        const course = await course_model_1.default.findById(courseId)
...
exports.getFullCourse = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const courseId = req.params.id;
